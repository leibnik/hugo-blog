...
3. 当 n > 2 时，情况又变得更为复杂，不妨讨论以下几种情形
	...
	**④** 根据前面的讨论，对于 ($x_1$, $x_2$, $x_3$)，其中 $x_1$ > 1，$x_2$ > 1，$x_3$ > 1 且 $x_1$ ≠ $x_2$ ≠ $x_3$，先手应避免直接使局面变为($x$, $x$, $y$)，($x$，$y$, 0) 其中 $x$ > 0, $y$ > 0 且 $x$ ≠ $y$，因为后手能使局面变为 ($x$，$x$, 0) 而轮到先手面对 ($x$，$x$, 0) 在后手采用必胜策略的情况先手必败。而获胜的关键恰恰就是迫使对方把局面变为 ($x$, $x$, $y$) 或 ($x$，$y$, 0)，具体举几个例子：
	```
	对于 (1, 2, 3) 这样的局面，先手按规则无论怎么拿，都会使局面转变为 ($x$, $x$, $y$)，($x$，$y$, 0) 其中 $x$ > 0, $y$ > 0 且 $x$ ≠ $y$，并且发现将各堆数进行异或运算后值为 0。
	```
	...
	7|1|1|1
	xor|0|0|0
	推导一番就会发现后手总有策略使先手下出 ($x$, $x$, $y$)，($x$，$y$, 0) 的局面
