...
对于采用 misere 规则的 ICG，区别只在于所有的终态都是 N 态。


## 定理及证明

**定理： 对于一个 Nim 游戏，局面为，当且仅当 nim-sum ≠ 0 时，先手有必胜策略。否则，后手有必胜策略。**

```
证明： 

注意到 nim-sum ($\oplus$) 满足类似加法的结合律与交换律，同时还满足两个相同的数异或结果为 0。我们以 $x_1$,..., $x_n$ 表示移动前的状态，以 $y_1$,...,$y_n$ 为移动后的状态，设 s = $x_1$ $\oplus$...$\oplus$$x_n$ ，t = $y_1$ $\oplus$...$\oplus$$y_n$，当移动的堆为第 k 堆，我们有 $x_i$ = $y_i$ 且 i ≠ k，$x_k$ > $y_k$。根据异或的特性可得：
...
有了前面对 nim 游戏的分析，我们直接分析 anti-nim 游戏 n 堆(n 大于等于 1 的整数)的情形: 
