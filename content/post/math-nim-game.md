...
## 前言
最近在公司活动中玩了一个叫 3，5，7 取物的博弈游戏，其规则是两位玩家轮流取物，每次取物只能在三堆物品中选择其中一堆并至少取走一个，可以直接一堆取完，当所有堆都取完时游戏即结束并且最后取完的输。
...
据了解这个游戏源自中国，后由到美洲打工的华人外传，之所以称为 Nim 有个说法称是由粤语的“拈”音译而得，Nim 游戏后来也演化出多个版本的玩法，它的必胜策略直到 20 世纪初才由一个叫 [Charles L. Bouton](https://en.wikipedia.org/wiki/Charles_L._Bouton) 的数学家在他的一篇论文中给出。论文地址：[Nim, A Game with a Complete Mathematical Theory](http://www.jstor.org/stable/1967631?origin=crossref&seq=1#page_scan_tab_contents)

## Nim Game
### 分析
首先尝试分析 Nim 游戏(以最后一个取完为获胜目标的玩法)的规律，以取石子为例，假定 w 为石子的总数(w >= 1)，这些石子被分为 n 堆(n >= 1)，局面状态以 ( $x_1$ , $x_2$, ...,$x_n$ ) 进行表示:
...
2. 当 n = 2 时，可以分为四种情况进行讨论
	...
	**④** ($x_1$, $x_2$) 局面， 其中 $x_1$ > 1 且 $x_2$ > 1，同时有 $x_1$ 不等于 $x_2$，此时先手必定能将局面转变为 **③** ，此时先手必胜；
	...
	针对以上 n = 2 时的两种情形的讨论可以发现，堆数为 2 时，两堆的数目的异或结果是否为 0 直接关系到先手是否能取胜（双方都采取最优策略的前提下），因此可以得到这样的结论：**当 n = 2 时，对于 ($x_1$, $x_2$)，$x_1$ > 0 且 $x_2$ > 0 ，利用异或操作符我们可以判断出先手的胜负，此时若 $x_1$ $\oplus$ $x_2$ = 0 ，则后手存在必胜策略，反之 $x_1$ $\oplus$ $x_2$ ≠ 0 则先手存在必胜策略。**
...
对于其他情况已经难以分析了，但也能隐约看出异或运算就是关键。假设 n = 2 时的结论对于 n > 2 时同样成立，那么对于 (1, 2, 3) 这样的局面，后手存在必胜策略。
...
到了这里分析已经难以继续进行，虽然以上分析不能最后解决问题，但依旧有着启发意义。
...
**定理： 对于一个 Nim 游戏，局面为 ($x_1$,..., $x_n$)，它是 P-position 当且仅当 nim-sum (即: $x_1$ $\oplus$...$\oplus$$x_n$) = 0，相对的，当 nim-sum ≠ 0 时，局面为 N-position。**
...
针对实际进行的 Nim 游戏，当玩家面对 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_k$ ... $\oplus$ $x_n$ = 0 的局面时，无论怎么取都会使 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $y_k$ ... $\oplus$ $x_n$ ≠ 0，而当玩家面对 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_k$ ... $\oplus$ $x_n$ ≠ 0 则总有办法使 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $y_k$ ... $\oplus$ $x_n$ = 0。对于 Nim 游戏有结论：**当 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0  则后手存在必胜策略，否则先手存在必胜策略，当 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ ≠ 0 时，先手若采取最优策略必胜，此时的必胜策略也是显而易见的，只要每次使 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0 把必败态交给对方即可取胜。**
...
1. 当 n 堆石子所有堆的石子的个数均为 1，即 (1, 1, ..., 1)：

	① 当 n 为偶数时，为先手的非必败态；

	② 当 n 为奇数时，为先手的必败态；

2. 当 n 堆石子中有且仅有一个石子数目大于 1 的堆，即 (1, ..., $x$, ...,1)：
...
3. 当 n 堆石子中存在两个以上石子数目大于 1 的堆，局面为 ($x_1$, $x_2$,..., $x_n$)：
	...
	① 当 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0 时，只可能向两种状态转变
	  ...
	  此时(a), (b) 两种状态对后手而言是非必败态，也就是说 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0 时，后手存在必胜策略。
...
结合以上几种情形可以得出结论: **当所有堆的石子数均为 1 且 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0 或 至少存在一个以上石子数目大于 1 的堆且 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ ≠ 0时，先手存在必胜策略，否则后手存在必胜策略。**

以 (3, 5, 7) 为例，符合至少存在一个以上石子数目大于 1 的堆且 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ ≠ 0，此时先手存在必胜策略。
...
而要注意的是，当局面转变为有且仅有一个石子数目大于 1 的堆，即 (1, ..., $x$, ...,1)时，策略就不再是使 nim_sum = 0，而是使所有堆的石子的个数均为 1且堆数 n 为奇数，确保了再次轮到自己时所有堆的石子数均为 1 且 $x_1$ $\oplus$ $x_2$ $\oplus$ ... $\oplus$ $x_n$ = 0。
